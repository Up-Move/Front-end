...
import ifsp.spo.edu.vagainclusivafrontend.userInterface.telaTokenCadastro.TelaTokenCadastro;
...
@SuppressWarnings({"Convert2Lambda", "SpellCheckingInspection", "NullableProblems"})
public class TelaCadastro extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
                ...
                if(event.getAction() == MotionEvent.ACTION_UP) {
                    if(event.getRawX() >= Senha.getRight()-Senha.getCompoundDrawables()[Right].getBounds().width()){
                        ...
                        if(senhaVisivel) {
                            ...
                            Senha.setCompoundDrawablesRelativeWithIntrinsicBounds(0, 0, R.drawable.icon_eye_close, 0);
                            ...
                            // Imagem que será desenhada quando senha não for mostrada
                            Senha.setCompoundDrawablesRelativeWithIntrinsicBounds(0, 0, R.drawable.icon_eye_open, 0);
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                if(event.getAction() == MotionEvent.ACTION_UP) {
                    if(event.getRawX() >= ConfirmarSenha.getRight()-ConfirmarSenha.getCompoundDrawables()[Right].getBounds().width()){
                        ...
                        if(senhaVisivel) {
                            // Imagem que será desenhada quando senha não for mostrada
                            ConfirmarSenha.setCompoundDrawablesRelativeWithIntrinsicBounds(0, 0, R.drawable.icon_eye_close, 0);
                            ...
                            // Imagem que será desenhada quando senha não for mostrada
                            ConfirmarSenha.setCompoundDrawablesRelativeWithIntrinsicBounds(0, 0, R.drawable.icon_eye_open, 0);
        ...
        btnRegister.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                TokenValidationCadastroAPI user = new TokenValidationCadastroAPI(nome, email, senha, confirmarsenha);
                ...
                requestBody.put("token", codigoAleatorio);

                Call<Void> callvoid = emailAPI.enviarEmail(requestBody);

                // Envia os Dados de Cadastro para a Proxima Pagina
                API API = retrofit.create(API.class);
                Call<JsonObject> call = API.validationUser(user);

                call.enqueue(new Callback<JsonObject>() {
                    @Override
                    public void onResponse(@NonNull Call<JsonObject>  call, @NonNull Response<JsonObject> response) {
                        if (response.isSuccessful()) {
                            Toast.makeText(TelaCadastro.this, "Valide seu Cadastro", Toast.LENGTH_SHORT).show();
                            Intent intent = new Intent(TelaCadastro.this, TelaTokenCadastro.class);
                            intent.putExtra("nome", nome);
                            intent.putExtra("email", email);
                            intent.putExtra("senha", senha);
                            intent.putExtra("confirmarsenha", confirmarsenha);
                            intent.putExtra("token_aleatorio", codigoAleatorio);

                            // Lógica para enviar Token por Email
                            callvoid.enqueue(new Callback<Void>() {
                                @Override
                                public void onResponse(Call<Void> call, Response<Void> response) {
                                    if (response.isSuccessful()) {
                                        Log.d("Validação do Token", "Token enviado com Sucesso");
                                    } else {
                                        // Erro Generico para caso ocorra outro tipo de Erro
                                        Toast.makeText(TelaCadastro.this, "Erro, Token Não Enviado", Toast.LENGTH_SHORT).show();
                                    }
                                }
    ...
    public static String gerarCodigoAleatorio() {
        ...
        for (int i = 0; i < 6; i++) {
            // Gere um caractere aleatório (número ou letra maiúscula)
            char caractereAleatorio = (char) (random.nextInt(26) + 'A'); // Letra maiúscula
            if (random.nextBoolean()) {
                // Se for verdadeiro, adicione um número aleatório
                caractereAleatorio = (char) (random.nextInt(10) + '0');
            }
